...
namespace PowerDev.Enterprise.Eureka.Extensoes
{
    public static class ListaExtensao
    {
        ...
        public static IEnumerable<IEnumerable<T>> QuebrarListas<T>(this IEnumerable<T> lista, int tamanho)
        {
            var proximoBloco = new List<T>(tamanho);
            foreach (var i in lista)
            {
                proximoBloco.Add(i);
                if (proximoBloco.Count != tamanho)
                    continue;
                yield return proximoBloco;
                proximoBloco = new List<T>(tamanho);
            }

            if (proximoBloco.Count > 0)
            {
                yield return proximoBloco;
            }
        }
